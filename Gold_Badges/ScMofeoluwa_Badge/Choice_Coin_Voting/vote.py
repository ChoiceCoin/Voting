...
algod_token = "ti4RqMynth6ecRMzEDRI414YbBppr4ociEOFKkLi"  # Put Algod Token here
headers = {"X-API-Key": algod_token}
# Initializes client for node.
algod_client = algod.AlgodClient(algod_token, algod_address, headers)

# Escrow creation.
escrow_address = "6KCOIOQB77UTHE67QUVJJ3NDGQCIBGO67KGOEYZZATIGK7KBU34EWG52AE"  # Put in main fund address here
escrow_mnemonic = "left later pencil gospel coast eight agree spatial raccoon absorb victory tell pioneer actor mouse refuse suggest help pledge tape oil drop gym absent raise"  # Put in main fund receiver_mnemonic here
...
def election_voting(vote):
    ...
    if vote == "YES":  # Add more boolean statements for more decisions or candidates.
        TX_ID = choice_vote(
            escrow_address, escrow_key, decision_one, 100, "Tabulated using Choice Coin"
        ...
        TX_ID = choice_vote(
            escrow_address, escrow_key, decision_two, 100, "Tabulated using Choice Coin"
        )
        message = (
            "Ballot Tabulated. \n You can validate that your vote was counted correctly at https://testnet.algoexplorer.io/tx/"
            + TX_ID[1]
...
def count_votes():
    ...
    if yes_count > no_count:
        if yes_count == 1:
            return "The Voting Process has ended. Candidate One received the most votes with {0} vote.".format(
                yes_count
            )
        else:
            return "The Voting Process has ended. Candidate One received the most votes with {0} votes.".format(
                yes_count
            )
    if no_count > yes_count:
        if no_count == 1:
            return "The Voting Process has ended. Candidate Two received the most votes with {0} vote.".format(
                no_count
            )
        else:
            return "The Voting Process has ended. Candidate Two received the most votes with {0} votes.".format(
                no_count
            )
...
def reset_votes():
    ...
    if yes_count > 0:
        transaction_2 = AssetTransferTxn(
            ...
            yes_count,
            choice_id,
            revocation_target=decision_one,
        )
    ...
    if no_count > 0:
        transaction_3 = AssetTransferTxn(
            ...
            no_count,
            choice_id,
            revocation_target=decision_two,
        )
...
def reset_corporate_votes():
    ...
    if yes_count > 0:
        transaction_2 = AssetTransferTxn(
            ...
            yes_count,
            choice_id,
            revocation_target=corporate_decision_one,
        )
        signature_2 = transaction_2.sign(clawback_key)
        algod_client.send_transaction(signature_2)
    if no_count > 0:
        transaction_3 = AssetTransferTxn(
            ...
            no_count,
            choice_id,
            revocation_target=corporate_decision_two,
        )
        signature_3 = transaction_3.sign(clawback_key)
        algod_client.send_transaction(signature_3)
    message = "Vote accounts reset. New Voting Process started."
