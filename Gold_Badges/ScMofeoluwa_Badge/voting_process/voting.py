...
fund_address = "WHNTB5KQTGKBQSZAJ5VX745SGYJTCKSQ2PX7KA3MVCXV7FNAEGLIGOKOZE"  # Put in main fund address here
fund_mnemonic = "foam fault power empty bulb usage round guard evoke city wish screen logic express assume extra copper kind prize table math wheat bargain absorb like"  # Put in main fund receiver_mnemonic here
...
def algo_inital(sender, key, receiver, comment):
    ...
    transaction = PaymentTxn(sender, parameters, receiver, 7000000, note=comment)
...
def create_optin(receiver_mnemonic, receiver_address, index):
    ...
    transaction = AssetTransferTxn(
        fund_address, parameters, receiver_address, 7, index
    )
